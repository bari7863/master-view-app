...
## ドキュメント同期のルール(必ず自動で行うこと)

コード変更(機能追加・仕様変更・DB構成変更・運用フロー変更・本番反映など)を行ったら、**ユーザーから指示されなくても**、作業完了時に必ず以下のドキュメントの更新要否を確認し、必要なら同じタイミングで更新してください。

- `README.md`(概要・セットアップ・環境変数・デプロイ方法)
- `docs/handover.md`(詳細仕様・進捗・既知の問題)
- `docs/roadmap.md` / `docs/roadmap.html`(ロードマップの進行状態)
- `docs/progress.md` / `docs/progress.html`(進捗ダッシュボード。Phase完了時・問題解決時はステータスを更新し、解決済みの既知の問題は記載を削除する)
- この `CLAUDE.md` 自身(重要ファイル一覧・DB構成・絶対ルールに変更が及ぶ場合)
- `directory-tree.txt`(ファイルの追加・削除時)

判断基準: 「新しい会話でこのプロジェクトを引き継いだ人が、これらのファイルだけ読んで迷わず作業を続けられるか」。ズレがあれば更新する。誤字修正のみ・内部リファクタのみで外部仕様に影響しない変更は更新不要。

ドキュメント更新自体は「作業完了時に自動的に確認・実施する」ものであり、hookのような完全自動化ではなく、このルールに従って都度判断・実行してください。
